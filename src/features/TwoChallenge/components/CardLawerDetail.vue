<template>
  ...
  <div class="text-white font-mono grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 items-center w-full gap-3">
    <section
      v-for="(lawer, key) in keys"
      :key="key"
    >
      <h1 class="font-nothing capitalize text-2xl">
        {{ key }}
      </h1>
      <div
        v-for="(field, index) in lawer"
        :key="index"
      >
        <div class="text-xs text-ellipsis overflow-hidden">{{ index }}: {{ field }}</div>
      </div>
    </section>
  </div>
</template>
